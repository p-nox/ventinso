...
export default function useUrlFilters(searchParams, navigate) {
  ...
  const [confirmedFilters, setConfirmedFilters] = useState(parseFiltersFromParams());

  useEffect(() => {
    const newFilters = parseFiltersFromParams();
    setFilters(newFilters);
    setConfirmedFilters(newFilters);
  }, [searchParams]);
  ...
  const updateUrl = (updated) => {
    ...
    if (updated.minPrice != null && updated.minPrice !== "") params.set("minPrice", updated.minPrice);
    if (updated.maxPrice != null && updated.maxPrice !== "") params.set("maxPrice", updated.maxPrice);
  ...
  const handleClearFilters = () => {
    const cleared = { q: "", category: null, type: null, condition: null, minPrice: "", maxPrice: "" };
    handleFilterChange(cleared);
  };

  return { filters, confirmedFilters, handleFilterChange, handleRemoveFilter, handleClearFilters };
}
